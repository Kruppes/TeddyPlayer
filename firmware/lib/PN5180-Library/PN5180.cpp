...
bool PN5180::transceiveCommand(uint8_t *sendBuffer, size_t sendBufferLen, uint8_t *recvBuffer, size_t recvBufferLen) {
  const uint32_t busyTimeoutMs = 50;
  auto waitForBusy = [&](int expected) {
    uint32_t start = millis();
    while (digitalRead(PN5180_BUSY) != expected) {
      if (millis() - start > busyTimeoutMs) {
        return false;
      }
    }
    return true;
  ...
  // 0.
  if (!waitForBusy(LOW)) return false;
  // 1.
  ...
  // 3.
  if (!waitForBusy(HIGH)) return false;
  // 4.
